	...
	*/
	function xmlIn($fname) {
		...
		foreach ($obj as $h) {
			...
			$ty = $j;
			foreach ($ty as $type) {
				if ($type->nodeName == "ObjectType" && ($type->nodeValue == 'Sets'
	...
	function xmlOut($obj, $dom, bool $bool = FALSE , $trm = null) {
		...
		if ($robj == 'mMap') {
			...
			for ($x = 0 ; $x < $obj->size() ; $x++) {
				...
				$tmp2 = $dom->createTextNode(key($obj->kv));
				$tmp2 = $tmp1->appendChild($tmp2);
				if (is_object($obj))
					$dom = $this->xmlOut(current($obj->kv),$dom, 1, $vpr);
				next($obj->kv);
			}
			return $dom;
		}
			...
			for ($x = 0 ; $x < $obj->size() ; $x++) {
				...
				$tmp4 = $dom->createTextNode(current($obj->kv));
				$tmp4 = $tmp3->appendChild($tmp4);
				next($obj->kv);
			}
			return $dom;
		}
		else if ($robj == 'Vector' || $robj == 'Vector') {
			$dom = $this->xmlOut($obj->dat, $dom,1);
			return $dom;
		}
		else if ($robj == 'Matrix' || $robj == 'Matrix') {
			$rpv = $dom->firstChild;
			$op = 0;
			for ($x = 0 ; $x < $obj->size() ; $x++)
