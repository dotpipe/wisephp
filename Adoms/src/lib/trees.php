...
class Trees extends Common
{
    ...
    public function linkTree($m)
    {
        ...
        for ($i = 0 ; $i < count($tmp) ; $i++) {
            ...
            if (preg_match("/[\[\{]/", $temp)) {
                ...
                $output = $output . '<br>';
            } elseif ($i + 1 < count($tmp) && preg_match("/[\"'][<>!#@?\,\\/%\-A-z0-9\s\._:]+[\"'][:$]/", $temp)) {
                $i++;
    ...
    public function create($va = 0)
    {
        ...
        for ($i = 0 ; $i < $va->size() ; $i++) {
            ...
            } elseif (count($temp) == 2 && !preg_match("/[<>!#@?\\/%\-A-z\s_]+/", $temp[1])
                && preg_match("/[0-9\.]+/", $temp[1], $tp)) {
                $outstring = $outstring . "\"" . $temp[0] . "\":" . $tp[0] . ",";
            } elseif (count($temp) == 2 && preg_match("/[\[\{]/", $temp[1])) {
                $outstring = $outstring . "\"" . $temp[0] . "\":" . $temp[1];
            } elseif (count($temp) == 2 && preg_match("/[nul\,]{4,5}/", $temp[1], $nul)) {
                $outstring = $outstring . "\"" . $temp[0] . "\":" . $temp[1];
            } elseif (count($temp) == 2 && count($checkBracket) == 1 && preg_match("/[\}\]]/", $checkBracket[0])) {
                $outstring = $outstring . "\"" . $temp[0] . "\":\"". $temp[1] . "\"";
            } elseif (count($temp) == 2 && count($checkBracket) == 1 && !preg_match("/[\}\]]/", $checkBracket[0])) {
                $outstring = $outstring . "\"" . $temp[0] . "\":\"". $temp[1] . "\",";
            } elseif (count($temp) == 2) {
                $outstring = $outstring . "\"" . $temp[0] . "\":\"". $temp[1] . "\",";
            } elseif (count($temp) == 1 && is_array($temp) && preg_match("/[\{\[]/", json_encode($temp), $ty)) {
                $outstring = $outstring . $ty[0];
            } elseif (count($temp) == 1 && is_array($temp) && preg_match("/[\}\]]/", json_encode($temp), $ty)) {
                $outstring = $outstring . $ty[0];
            } elseif (count($temp) == 1 && preg_match("/[\{\[]/", $temp[0], $ty)) {
                $outstring = $outstring . $ty[0];
            } elseif (count($temp) == 1 && preg_match("/[\}\]]/", $temp[0], $ty)) {
                $outstring = $outstring . $ty[0];
            } elseif (count($temp) == 1 && count($checkBracket) == 1 && !preg_match("/[\}\]]/", $checkBracket[0])) {
